...
const IconSpinner = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" className="spin-icon">
    ...
    <div className="pp-modal-overlay" onClick={onClose}>
      <div className="pp-modal" onClick={(e) => e.stopPropagation()}>
        <div className="pp-modal-header">
          ...
          <button className="pp-modal-x" onClick={onClose}><IconClose /></button>
        </div>
...
const PendingPayouts = () => {
  ...
  const handleModalClose = () => {
    setModal(null);
    if (modalResolve) modal?.type === "prompt" ? modalResolve(null) : modalResolve();
  };

  const handleModalConfirm = () => {
    setModal(null);
    if (modalResolve) modal?.type === "prompt" ? modalResolve(modalInput) : modalResolve();
  };
  ...
  const handlePageChange = (newPage) => {
    ...
    if (newPage >= 1 && newPage <= totalPages) fetchPayouts(newPage, pagination.limit);
  };
  ...
  const handleFileUpload = async (payoutId, event) => {
    ...
    if (files.some((f) => !allowed.includes(f.type))) {
      await showAlert("Invalid File Type", "Please select only PDF or image files (JPG, PNG).", "⚠️");
      event.target.value = "";
      return;
    }
    ...
    try {
      ...
      const msg =
        error.response?.status === 404
          ...
          : error.response?.data?.error || error.message || "Failed to upload transfer proof.";
  ...
  const handleProcessPayout = async (payout) => {
    ...
    try {
      ...
      await showAlert("Payout Completed", result.message || "Payout marked as completed!", "🎉");
    } catch {
      await showAlert("Error", "Failed to process payout. Please try again.", "❌");
    } finally {
    ...
    <div className="pending-payouts-container">
      ...
      <div className="container">
        <div className="header">
          ...
          <p className="subtitle">Manage and process pending payouts for instructors and DJs</p>
        </div>
                      ...
                      <tr key={payout.id} className="table-row">
                        <td>
                          <div className="user-info">
                            <div className="user-avatar">
                              {payout.user.avatar
                                ? <img src={payout.user.avatar} alt={payout.user.name} className="avatar-img" />
                                : <div className="avatar-placeholder">👤</div>}
                            </div>
                            <div className="user-details">
                              <div className="user-name">{payout.user.name}</div>
                              <div className="user-email">{payout.user.email}</div>
                              <span className={`user-type ${payout.user.type}`}>{payout.user.type}</span>
                            </div>
                          </div>
                        </td>
                        <td>
                          <div className="amount-info">
                            <div className="amount">{formatCurrency(payout.amount)}</div>
                            <div className="transaction-count">{payout.transaction_count} transactions</div>
                          </div>
                        </td>
                        <td>
                          <div className="payment-method">
                            <i className="payment-icon">💳</i>
                            <span className="method-text">{payout.payment_method.replace("_", " ")}</span>
                          </div>
                        </td>
                        <td>
                          <div className="payment-details">
                            <div><strong>Bank:</strong> {payout.payment_details.bank_name}</div>
                            <div><strong>Account:</strong> ****{payout.payment_details.account_number.slice(-4)}</div>
                            <div><strong>Holder:</strong> {payout.payment_details.account_holder_name}</div>
                            <div><strong>Routing:</strong> {payout.payment_details.routing_number}</div>
                          </div>
                        </td>
                        <td>
                        ...
                        <td>
                          <span className={`days-pending ${
                            payout.days_pending > 7 ? "urgent" : payout.days_pending > 3 ? "warning" : "normal"
                          }`}>
                          ...
                          <div className="transfer-proof">
                            {payout.has_transfer_proof ? (
                              <div className="proof-uploaded">
                                <span className="proof-status uploaded">✓ Uploaded</span>
                                {payout.transfer_proof_urls?.length > 0 && (
                                  <div className="proof-links">
                                    {payout.transfer_proof_urls.map((url, i) => (
                                      <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="proof-link">
                                        View Proof {i + 1}
                                      </a>
                                    ))}
                                  </div>
                              ...
                              <div className="proof-missing">
                                <span className="proof-status missing">✗ Missing</span>
                                <div className="upload-section">
                                  <input
                                    type="file" multiple accept=".pdf,.jpg,.jpeg,.png"
                                    onChange={(e) => handleFileUpload(payout.id, e)}
                                  ...
                                  >
                                    {uploadingProof[payout.id] ? "Uploading..." : "Upload Proof"}
                                  </label>
                                </div>
                              </div>
                            )}
                        ...
                        <td>
                          <div className="action-buttons">
                            <button
                              ...
                              title="Mark as Paid"
                            >
                              {processingPayout[payout.id] ? <IconSpinner /> : <IconMarkPaid />}
                            </button>
                          </div>
                        </td>
                      </tr>
