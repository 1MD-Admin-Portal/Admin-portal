...
import {
  ...
  Plus, Search, X, AlertCircle, Flag, MessageSquare,
} from "lucide-react";
...
const DanceStylesSelect = ({ selected, onChange, options }) => {
  ...
  return (
    <div className="ds-wrap" ref={ref}>
      <button type="button" onClick={() => setOpen(v => !v)} className={`ds-trigger ${open ? "ds-trigger--open" : ""}`}>
      ...
      {open && (
        <div className="ds-dropdown">
          <div className="ds-search-wrap">
            <input type="text" className="ds-search" value={query}
              onChange={e => setQuery(e.target.value)} placeholder="Search styles..." autoFocus />
          </div>
          <div className="ds-options">
            {filtered.map(d => {
              ...
              return (
                <label key={d.id ?? name} className={`ds-option ${checked ? "ds-option--checked" : ""}`}
                  onClick={() => toggle(name)}>
                  <div className="ds-option__left">
                    <input type="checkbox" checked={checked} onChange={() => {}} className="ds-checkbox" />
                    {name}
                  </div>
            ...
            {!options.length && <div className="ds-empty">No dance styles available.</div>}
            {!!options.length && !filtered.length && <div className="ds-empty">No matches.</div>}
          </div>
          <div className="ds-footer">
            <button type="button" className="ds-clear" onClick={() => onChange([])}>Clear</button>
            <button type="button" className="ds-done" onClick={() => setOpen(false)}>Done</button>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="modal-shell" style={{ maxWidth: "420px" }}>
      ...
      <div className="modal-body-custom">
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem", padding: "0.5rem 0", textAlign: "center" }}>
          ...
          <p style={{ margin: 0, fontSize: "0.9rem", color: "#374151", lineHeight: 1.6 }}>{message}</p>
        </div>
      </div>
...
const ChallengePage = () => {
  ...
  const [tasks, setTasks] = useState([{ task_type: "watch_video", task_title: "", video_url: "" }]);
  ...
  const fetchPendingSubmissions = async () => {
    try { setLoading(true); const r = await getPendingSubmissionsService(page, 20); setPendingSubmissions(r.submissions || []); }
  ...
  const openSubmissions = async (challenge) => {
    try { setSelectedChallenge(challenge); const r = await getChallengeSubmissionsService(challenge.id); setSubmissions(r.submissions || []); setSubmissionsModal(true); }
    catch { setError("Failed to fetch submissions"); }
  };

  const openParticipants = async (challenge) => {
    try { setSelectedChallenge(challenge); const r = await getChallengeParticipantsService(challenge.id); setParticipants(r.participants || []); setParticipantsModal(true); }
    catch { setError("Failed to fetch participants"); }
  };

  const openAnalytics = async (challenge) => {
    try { setSelectedChallenge(challenge); const r = await getChallengeAnalyticsService(challenge.id); setAnalytics(r); setAnalyticsModal(true); }
  ...
  const handleRejectSubmission = async (id) => {
    try { await rejectSubmissionService(id, feedbackText); setSuccess("Submission rejected"); setFeedbackText(""); setSubmissionDetailModal(false); fetchPendingSubmissions(); }
  ...
  const handleFileUpload = async (event, fieldName, taskIndex = null) => {
    ...
    try {
      ...
      if (fieldName === "image_url") setNewChallenge(prev => ({ ...prev, image_url: url }));
      else if (fieldName === "video_url" && taskIndex !== null) updateTask(taskIndex, "video_url", url);
      setSuccess("File uploaded successfully");
    } catch { setError("Failed to upload file"); } finally { setUploading(false); }
  };
  ...
  const handleCreateChallenge = async () => {
    try {
      await createChallengeService({
        ...newChallenge,
        dance_style: Array.isArray(newChallenge.dance_style) ? newChallenge.dance_style.join(", ") : newChallenge.dance_style,
        tasks: tasks.filter(t => t.task_title.trim() !== ""),
      });
      setSuccess("Challenge created successfully");
      setCreateChallengeModal(false);
      setNewChallenge({ title: "", challenger_type: "public", description: "", image_url: "", dance_style: [], dance_level: "", start_date: "", end_date: "", prize_details: "", max_participants: "", status: "draft", is_trending: false });
                  ...
                  <div key={challenge.id} className="fp-content-card" onClick={() => openChallengeDetails(challenge)}>
                    ...
                    <div className="fp-thumbnail-wrap">
                      {challenge.image_url ? (
                        ...
                        <div className="fp-media-thumb fp-media-placeholder"><Award size={36} /></div>
                      )}
                ...
                <div key={p.user_id} className="participant-item">
                  ...
                  <div className="participant-info">
                    ...
                    <div className="participant-stats">
                      <span className="participant-stat-date">Joined: {new Date(p.joined_at).toLocaleDateString()}</span>
                      <span className="participant-stat-sub"><Upload size={11} /> {p.submissions_count || 0} submissions</span>
                      <span className="participant-stat-like"><ThumbsUp size={11} /> {p.likes_received || 0} likes</span>
                    </div>
                  </div>
                  <button className="remove-participant-btn" onClick={() => handleRemoveParticipant(selectedChallenge.id, p.user_id)}>
